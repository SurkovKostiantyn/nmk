...
[Lab 13](https://github.com/SurkovKostiantyn/nmk/tree/master/cloud_technologies/projects/lab_13_start_project) — CI/CD (GitHub Actions)
...
Бейдж статусу (**status badge**) — це динамічне зображення, яке відображає поточний стан вашого пайплайну. Він дозволяє швидко перевірити працездатність коду прямо на головній сторінці репозиторію в GitHub.

**Навіщо це потрібно?**

- **Швидка діагностика** - вам не потрібно заходити у вкладку "Actions", щоб дізнатися, чи працює ваш код.
- **Довіра** - користувачі або колеги бачать, що проєкт підтримується в робочому стані.
- **Автоматизація** - картинка оновлюється автоматично після кожного комміту.

Коли ви відкриваєте головну сторінку репозиторію, ви одразу бачите статус:

- 🟢 **passing** - останній запуск тестів та деплою пройшов успішно.
- 🔴 **failing** — щось зламалося (тести не пройшли або збірка завершилася помилкою).
- 🟡 **in progress** — пайплайн зараз виконується.

**Де взяти бейдж?**

1. Перейдіть у вкладку **Actions** вашого репозиторію.
2. Оберіть ваш workflow (наприклад, "CI/CD Pipeline") у списку зліва.
3. Натисніть на кнопку з трьома крапками `...` у правому верхньому куті.
4. Оберіть **Create status badge**.
5. Скопіюйте Markdown-код та вставте його у ваш файл `README.md`.

Приклад того, як це виглядає в `README.md`:

```markdown
# Lab 13 — CI/CD

![CI/CD Pipeline](https://github.com/<username>/lab13-cicd/actions/workflows/ci-cd.yml/badge.svg)
```
