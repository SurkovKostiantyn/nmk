...
Після підготовки коду та Dockerfile, необхідно зібрати образ та перевірити роботу застосунку у локальному контейнері.

1. **`docker build`** — створює Docker-образ на основі інструкцій з `Dockerfile`. Прапор `-t` задає тег (ім'я) образу у форматі `користувач/репозиторій:версія`. Крапка `.` в кінці вказує на поточну директорію як контекст збірки.
2. **`docker run`** — створює та запускає контейнер.
   - `-d` (detached) — запуск у фоновому режимі.
   - `--name` — задає зручне ім'я для керування контейнером.
   - `-p 8080:3000` — прокидає порт 3000 з контейнера на порт 8080 хоста.
   - `-e` — передає змінні оточення (environment variables).
3. **`docker exec`** — дозволяє підключитися до працюючого контейнера. Команда `/bin/sh` відкриває інтерактивну оболонку всередині ізольованого середовища.

```bash
# Збірка образу (замість <username> — ваш Docker Hub логін)
docker build -t <username>/lab06-app:1.0 .

# Перегляд створеного образу
docker images | grep lab06
...
# Вхід у контейнер для дослідження
docker exec -it lab06 /bin/sh

# Зупинка та видалення
docker stop lab06 && docker rm lab06
```

#### Крок 4.1. Автоматизація за допомогою Docker Compose

Docker Compose дозволяє описати всі параметри запуску (порти, змінні оточення, контекст збірки) у одному файлі `docker-compose.yml`.
...
# Приклади команд всередині контейнера:
# ls -la          — перегляд файлів
# env             — перегляд змінних оточення
# hostname        — перегляд імені хоста
# exit            — вихід з контейнера

Приклад роботи команд всередині контейнера:

![alt text](media/lab6_screen6.png)
