...
9. Препроцесори та постпроцесори
...
- **Блок (Block):** Незалежний компонент (напр., `.card`, `.button`, `.menu`).
- **Елемент (Element):** Частина блоку, яка не має сенсу сама по собі (`.card__image`, `.card__title`). Використовується подвійне підкреслення `__`.
- **Модифікатор (Modifier):** Змінює зовнішній вигляд або стан (`.card--hot`, `.button--disabled`). Використовується подвійний дефіс `--`.
...
- **Структура:** `.btn` (розміри, відступи).
- **Вигляд:** `.btn-blue` (колір, градієнт).
  _Результат:_ Менше повторів коду (DRY - Don't Repeat Yourself).
...
- **Плюси:** Неймовірна швидкість розробки, відсутність потреби придумувати назви класів.
- **Мінуси:** HTML-код стає дуже "засміченим" довгими рядками класів.
...
- **Що це:** Мови, які розширюють можливості CSS, дозволяючи використовувати змінні, вкладеність, функції та інші конструкції, яких немає в чистому CSS.
- **Як працюють:** Пишеться код на препроцесорі (наприклад, `.scss`), який потім компілюється (транспілюється) в звичайний CSS, зрозумілий браузеру.
- **Приклад (Sass):**
...
### Постпроцесори (PostCSS, Autoprefixer)

- **Що це:** Інструменти, які обробляють уже готовий CSS код, покращуючи його або додаючи необхідні функції.
- **Як працюють:** Приймають CSS і видають оптимізований CSS.
- **Приклад (PostCSS + Autoprefixer):** Автоматичне додавання вендорних префіксів для підтримки старих браузерів:
